...
async function registerCommands() {
  ...
  const commands = [
    new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName('deadline').setDescription('Termín (DD.MM.YYYY)').setRequired(true)),
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'mytasks') {
    ...
    tasks.forEach(t => {
      embed.addFields({
        name: `#${t.id} – ${t.title}`,
        value: `${t.area} | Deadline: ${formatDate(t.deadline)} | ${STATUS_CS[t.status]}`,
      });
    });
  ...
  if (commandName === 'tasks') {
    ...
    tasks.slice(0, 20).forEach(t => {
      embed.addFields({
        name: `#${t.id} – ${t.title}`,
        value: `${t.assigneeName} | ${t.area} | Deadline: ${formatDate(t.deadline)} | ${STATUS_CS[t.status]}`,
      });
    });
...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  try {
    ...
    const taskId = row[0];
    const currentStatus = row[5]; // Stav je nyní sloupec F (index 5)
