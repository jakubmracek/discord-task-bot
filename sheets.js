...
// A    B       C              D               E         F      G       H         I          J
// ID | Název | Přiřazeno | Discord ID | Deadline | Stav | Oblast | Zadal | Zadáno dne | Link
const COLUMNS = {
  ID: 0, TITLE: 1, ASSIGNEE_NAME: 2, ASSIGNEE_ID: 3,
  DEADLINE: 4, STATUS: 5, AREA: 6,
  CREATED_BY: 7, CREATED_AT: 8, MESSAGE_LINK: 9,
};
...
async function updateTaskStatus(taskId, newStatus) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: process.env.SHEET_ID,
    range: `${process.env.SHEET_NAME}!F${sheetRow}`,
    valueInputOption: 'RAW',
...
async function updateTaskMessageId(taskId, messageId) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: process.env.SHEET_ID,
    range: `${process.env.SHEET_NAME}!J${sheetRow}`,
    valueInputOption: 'RAW',
    requestBody: { values: [[messageId]] },
  });
}

// ── Najít úkol podle Discord message ID (sloupec J) ───────────────────────────
